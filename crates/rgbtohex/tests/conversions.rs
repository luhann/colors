...
fn test_parse_rgb_errors() {
    ...
    assert_eq!(parse_rgb("255,0,0,2.0"), Err("Alpha must be between 0.0 and 1.0"));
...
fn test_convert_rgb_to_format_hex() {
    ...
    assert_eq!(convert_rgb_to_format("255, 0, 0, 0.5", "hex").unwrap(), "#FF00007F");
}

#[test]
fn test_convert_rgb_to_format_other_formats() {
    assert_eq!(convert_rgb_to_format("255,0,0", "standard").unwrap(), "RGB(255, 0, 0)");
    assert_eq!(convert_rgb_to_format("255,0,0,0.67", "css").unwrap(), "rgba(255, 0, 0, 0.67)");
    assert_eq!(convert_rgb_to_format("255,0,0", "json").unwrap(), r#"{"r": 255, "g": 0, "b": 0}"#);
    assert_eq!(convert_rgb_to_format("255,0,0,0.67", "compact").unwrap(), "255,0,0,0.67");
}

#[test]
fn test_convert_rgb_to_format_errors() {
    assert_eq!(convert_rgb_to_format("255,0,0", "invalid").unwrap_err(), "Unknown format: invalid");
    assert_eq!(convert_rgb_to_format("255,0", "hex").unwrap_err(), "Invalid RGB format");
}
