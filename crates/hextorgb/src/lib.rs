//! # hextorgb
//!
//! Fast, zero-dependency hex-to-RGB color conversion library.
//!
//! ## Features
//! - Parse hex colors with or without alpha channel
//! - Convert to multiple output formats (RGB, CSS, JSON, hex, compact)
...
//!
//! let result = hextorgb("#FFFFFF", "standard").unwrap();
...
/// Parse a hex color string into RGB components and optional alpha channel.
///
/// Accepts formats: `#RRGGBB`, `#RRGGBBAA`, `0xRRGGBB`, `0xRRGGBBAA`, or plain `RRGGBB`/`RRGGBBAA`.
///
/// # Arguments
/// * `hex` - A hex color string
///
/// # Returns
/// * `Ok(([r, g, b], alpha))` - RGB triple and optional alpha byte
/// * `Err(msg)` - Error message if parsing failed
///
/// # Examples
/// ```ignore
/// assert_eq!(parse_hex("#FFFFFF"), Ok(([255, 255, 255], None)));
/// assert_eq!(parse_hex("#FF000080"), Ok(([255, 0, 0], Some(128))));
/// ```
...
/// Convert a hex color string to various output formats.
///
/// Supports formats: `standard`, `css`, `json`, `hex`, `compact`.
///
/// # Arguments
/// * `hex` - Hex color string (e.g., `#FFFFFF` or `F0F`)
...
/// # Examples
/// ```ignore
/// assert_eq!(convert_hex_to_format("#FFFFFF", "standard"), Ok("RGB(255, 255, 255)".to_string()));
/// assert_eq!(convert_hex_to_format("#FFFFFF", "css"), Ok("rgb(255, 255, 255)".to_string()));
/// ```
